...
function size(name, args) {
  if (!args.length || args.length > 2)
    throw new SyntaxError("Must have one or two arguments.");
  const [blockSize, inlineSize] = args;
  const res = {};
  blockSize && (res.height = res.blockSize = blockSize);
  inlineSize && (res.width = res.inlineSize = inlineSize);
  return res;
}
...
const SHORTS = [
  ...
  [/^_flex(-[^(]+)?\(shrink/, (name, args) => ({ ["flex-" + name]: args[0] }), , ALIGN_WORDS],
