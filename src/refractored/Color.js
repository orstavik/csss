/**
 * Color space conversion helpers used by CSSS.
 *
 * This module provides pure functions to move between:
 * - sRGB hex (with/without alpha)
 * - non-linear sRGB (r,g,b 0–255)
 * - linear RGB (R,G,B 0–1)
 * - XYZ
 * - Lab
 * - LCH
 *
 * Public API:
 * - fromHex8, fromHex6, fromRgb, fromLinearRgb, fromXyz, fromLab, fromLCH
 *   Each returns a rich color object containing both perceptual (L,C,H) and
 *   hex/alpha representations.
 */
...
// --- Public API: conversion entry-points ---

/** Runs a list of transform steps, merging each result into one color object. */
function makeColor(transforms, all) {
  return transforms.reduce((acc, fn) => ({ ...acc, ...fn(acc) }), all);
}

/** Creates a full color object from 8-digit hex (#RRGGBBAA). */
export function fromHex8(hex8) {
  return makeColor([hex8ToHex6A, hex6ToRgb, rgbToRGB, RGBToXyz, xyzToLab, labToLch], { hex8 });
}

/** Creates a full color object from 6-digit hex (#RRGGBB) and optional alpha. */
...
/** Creates a full color object from sRGB (0–255 per channel) and optional alpha. */
...
/** Creates a full color object from linear RGB (0–1 per channel) and optional alpha. */
...
/** Creates a full color object from XYZ coordinates and optional alpha. */
...
/** Creates a full color object from Lab coordinates and optional alpha. */
...
/** Creates a full color object from LCH coordinates and optional alpha. */
