/**
 * CSSS Parser (v2):
 * - Tokenizes and parses CSSS short expressions (e.g., `$bg(red, cover)`).
 * - Resolves short names against the SHORTS vocabulary.
 * - Generates CSS rules (with selectors, layers, and media queries).
 *
 * This is the second iteration of the parser with improvements over Parser.js.
 *
 * Public API:
 * - `parse(shortExpression)`: parses a short string into an array of CSS rule objects.
 * - `sequence(allShorts, classListShorts, parseFun)`: reorders class list to match
 *   layer precedence.
...
function serializeCssBlock(name, body, depth = 0) {
  const spaces = "  ".repeat(depth);
  const spaces2 = "  ".repeat(depth + 1);
  const body2 = Object.entries(body).map(([k, v]) =>
    v instanceof Object ?
      serializeCssBlock(k, v, depth + 1) :
      `${spaces2}${k}: ${v};`
  ).join("\n");
  return `${spaces}${name} {\n${body2}\n${spaces}}`;
}
...
export function parse(short) {
  ...
  const cssText = serializeCssBlock(`@layer ${layer}`, obj);
  const main = { short, layer, media, selector, body, cssText };

  const atRuleTexts = Object.entries(atRules).map(([rule, body]) => ({ rule, body, cssText: serializeCssBlock(rule, body) }));
...
const mergeOrStackProperties = (function () {
  ...
  return function mergeOrStackProperties(interpretedShorts) {
    const res = {};
    for (const obj of interpretedShorts) {
      for (let [k, v] of Object.entries(obj)) {
        if (v == null) continue;
        if (!(k in res))
          res[k] = v;
...
function parseArgumentList(tokens, divider) {
  const args = [];
  if (tokens[0] == ")" && tokens.shift())
    return args;
  while (tokens.length) {
    const a = parseOperatorExpression(tokens)
    args.push(a);
...
function parseOperatorExpression(tokens) {
  let a = parsePrimaryExpression(tokens);
  while (tokens.length) {
    const { kind, text } = tokens[0];
    if (kind === "OPERATOR")
      a = operatorPriority(a, tokens.shift().text, parsePrimaryExpression(tokens));
    else if (kind === "NUMBER" && (text[0] == "-" || text[0] == "+"))
      a = { kind: "EXP", name: "+", args: [a, parsePrimaryExpression(tokens)] };
    else if (kind == "COLOR" && (a.type == "color" || a.kind == "COLOR"))
      a = { kind: "EXP", name: "#hash", type: "color", args: [a, parsePrimaryExpression(tokens)] };
    else
      break;
  }
  return a;
...
function parsePrimaryExpression(tokens) {
  if (tokens[0].text === "(")
    return { kind: "EXP", name: tokens.shift().text, args: parseArgumentList(tokens, ",") };
  ...
  if (tokens[1].text === "(") {
    ...
    tokens.shift();
    return kind == "COLOR" ?
      { kind: "EXP", type: "color", name, args: parseArgumentList(tokens, ",") } :
      { kind: "EXP", name, args: parseArgumentList(tokens, ",") };
  }
...
function parseNestedExpression(shortExp) {
  ...
  try {
    ...
    const short = parsePrimaryExpression(newTokens);
