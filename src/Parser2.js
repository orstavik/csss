...
function extractAtRules(obj, selector, media, layer) {
  const atRules = {}, mainRule = {};
  for (let [k, v] of Object.entries(obj)) {
    k[0] == ">" ? atRules[selector + k] = v :
      k.startsWith("::") ? atRules[selector + k] = v :
        (k[0] == "@" || k[0] == ":") ? atRules[k] = v :
          mainRule[k] = v;
  }
...
//todo if this is the 
function makeRule(selector, body, media, layer) {
  checkProperty(body);
  let obj = { [selector]: body };
  if (media) obj = { [`@media ${media}`]: obj };
  const cssText = bodyToTxt(`@layer ${layer}`, obj);
  return { short, layer, media, selector, body, cssText };
}
...
export function parse(short) {
  ...
  const Default = short.match(/^(\$|\|\$|\|\|\$)/) ? "Default" : "";
  const layer = (grandItem ? "grandItems" : item ? "items" : "container");
  exprList = kebabcaseKeys(exprList);
  const { atRules, mainRule: body } = extractAtRules(exprList, selector);
  checkProperty(body);
  let obj = { [selector]: body };
  if (media) obj = { [`@media ${media}`]: obj };
  const cssText = bodyToTxt(`@layer ${layer + Default}`, obj);
  const main = { short, layer: layer + Default, media, selector, body, cssText };
