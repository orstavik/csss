...
export function parse(short) {
  ...
  let first$ = exp.indexOf("$");
  let sel = exp.slice(0, first$);
  let exprList = exp.slice(first$ + 1);
  // let [sel, ...exprList] = exp?.split("$");
  exprList = parseNestedExpression(exprList);
...
function compatibleNumbers(args) {
  const { type, unit } = args[0];
  const res = [];
  for (let a of args) {
    if (a.type != type)
      throw new SyntaxError(`Incompatible types: ${args.map(a => a.text).join(", ")}`);
    if (a.unit == unit) {
      res.push(a.num);
      continue;
    }
    const factor = FACTORS[unit + "_" + a.type] ?? (1 / FACTORS[a.type + "_" + type]);
    if (factor) {
...
function computeNumbers(calc, a, c) {
...
//clamp() and round()-with-more-than-one-argument are different in css from js. the rest are equal on the number.
...
class OpExpression extends Expression {
  ...
  interpret(scope) {
    debugger
    const args = this.args.map(x => x instanceof Expression ? x.interpret(scope) : x);
    const numbers = compatibleNumbers(args.filter(a => a.kind === "NUMBER"));
    if (numbers)
      return computeNumbers(this.calc, ...args) ??
        `calc(${args.map(x => x?.text ?? x).join(" " + this.name + " ")})`;
  }

  append(right) {     //we always have left!
    if (this.pri >= (right.pri ?? 0)) {
      this.args[1] = right;
      return this;
    }
    debugger;
    let parent = this, child = this.args[1];
    while (child instanceof OpExpression && child.pri > right.pri) {
      parent = child;
      child = parent.args[1];
    }
    right.args[0] = child;
  ...
  prepend(left) {
    if (this.pri >= (left.pri ?? 0)) {
      this.args[0] = left;
      return this;
    }
    debugger
    let parent = left, child = parent.args[1];
    while (child instanceof OpExpression && child.pri > left.pri) {
      parent = child;
      child = parent.args[1];
    }
    child.args[1] = child;
  ...
  static extractSignFromRightsideNumber(num) {
    const sign = num.text[0];
    const right = { ...num, text: num.text.slice(1) };
    if (sign == "-") right.num *= -1;
    return new OpExpression(undefined, sign, right);
  }
}
...
function goRightOperator(tokens) {
  ...
  while (tokens.length && tokens[0].kind !== "SYN") {
    if (!(a instanceof OpExpression || a.kind === "NUMBER" || a.kind === "VAR"))
      throw new SyntaxError(`can't do ${a}-><-${tokens[0].text}`);
    let b = goDeep(tokens);
    if (b.kind === "NUMBER" && (b.text[0] == "-" || b.text[0] == "+"))
      b = OpExpression.extractSignFromRightsideNumber(b);
    if (b instanceof OpExpression && b.left === undefined)
      a = b.prepend(a);
    else
...
function goDeep(tokens) {
  ...
  if (tokens[1].kind === "OP")
    return new OpExpression(tokens.shift(), tokens.shift().text, goRightOperator(tokens));
  ...
  if (tokens[1].text === "(" && (tokens[0].kind === "NUMBER" || tokens[0].kind === "VAR"))
    return new OpExpression(tokens.shift(), "*", ...goRightComma(tokens, ""));
  if (tokens[1].text === "(" && tokens[0].math) //&& tokens[0].kind === "WORD"
    return new OpExpression(tokens.shift().text, tokens.shift().text).append(goRightComma(tokens, ","));
...
function parseNestedExpression(shortExp) {
  ...
  while (newTokens.length) {
    let short;
    try {
      short = goDeep(newTokens);
    } catch (e) {
      e.message += `\n${short}\n${" ".repeat(short.length - newTokens.reduce(((acc, { text }) => acc + text.length), 0))}^`;
      console.error(e);
      debugger
      throw e;
    }
    if (!(short instanceof Expression))
      throw new SyntaxError("not a valid short: " + short);
    if (newTokens.length)
      throw new SyntaxError("too many tokens after end parenthesis: " + newTokens.map(t => t.text).join(""));
    if (newTokens.length && newTokens[0].text !== "$")
      throw new SyntaxError("Expected $ between short expressions, got: " + newTokens[0].text);
    shorts.push(short);
  }
  return shorts;
}
...
export const TYPES = {
  ...
  MATH: "min|max|clamp|round|atan2|ceil|floor|abs|sin|cos|tan|asin|acos|atan|sqrt|log2|log10|exp",
  // MULTIMATH: "min|max|clamp|round|pow|atan2",
  // OTHER_FUNCTIONS: "url|attr|var|env|counter|counters|rect|repeat|minmax",
  SINCOSTAN: "sin|cos|tan", //must normalize the angle to rad first
  A_SINCOSTAN: "asin|acos|atan", //returns in rad
  ATAN2: "atan2",
  ABS_CEIL_FLOOR: "abs|ceil|floor",
  SQRT_LOG2_LOG10_EXP: "sqrt|log2|log10|exp",
  MINMAX: "min|max|clamp",
  CLAMP: "clamp",
  ROUND: "round",
};

// class SqrtLogExp extends OpExpression {
//   constructor(arg) {    super(arg, "sqrt");  }

//   interpret(scope) {
//     if(this.args.length != 1)
//       throw new SyntaxError(`${this.name}() takes exactly one argument.`);
//     if(this.args[0].kind === "VAR")
//       return `var(${this.args[0].text})`;
//     const a = this.args[0] instanceof Expression ? this.args[0].interpret(scope) : this.args[0];
//     if (a.kind === "NUMBER") {
//       if (a.num < 0)
//         throw new SyntaxError("sqrt of negative number");
//       return { ...a, num: Math.sqrt(a.num), text: Math.sqrt(a.num) + (a.unit || "") };
//     }
//     return `sqrt(${a.text || a})`;
//   }
// }
...
const tokenize = (_ => {
  ...
  return function tokenize(input) {
    ...
    for (let m; (m = TOKENS.exec(input));) {
      const [text, _, q, quote, ws, n, num, length, angle, time, percent, fr, vari, word, colorWord, c, c0, p0, c1, p1, c2, p2, c3, c4, p3, operator, /*coalesce, multdiv, plusminus,arrow,*/ cpp] = m;
      if (ws) continue;
      else if (num) {
      ...
      else if (word) out.push({ text, kind: "WORD", math: word.match(new RegExp(`^(${TYPES.MATH})$`)) });
      else if (colorWord) out.push({ text, kind: "COLORWORD" });
      else if (operator) out.push({ text, kind: "OP" });
