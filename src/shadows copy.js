...
//each shade consists of five properties: 
//   lengthFactor, blurFactor, spreadFactor, ratio for how much faster length/spread grows than blur, and color
//   Many of the shades do not require spread. 

const SHADES = {
  ambient: { length: 0.2, blur: 1.5, ratio: 1, color: "#0003" },
  ambients: { length: 0.2, blur: 1.5, spread: 1.25, ratio: 1, color: "#0003" },
  glow: { l: 0, b: .5, s: -.2 },
...
function parseNamedShadowArgs(args) {
  ...
  const argColor = extractColor(args);
  if (args.length)
  ...
  const color = argColor ?? type.color ?? "#0003";

  const rad = angle ?? (Math.PI * .75); //default angle 135deg
  const x = -round(Math.cos(rad) * type.length * num) + unit;
  const y = round(Math.sin(rad) * type.length * num) + unit;
  const blur = type.blur ? round(type.blur * num) + unit : undefined;
  const spread = type.spread ? round(type.spread * num) + unit : undefined;
